...
inputFrame.place(relx=0.98, rely=0.63, anchor=E)
inst1 = Label(root, text="How to Use:", font=("Roboto", 11, BOLD)).place(x=540, y=15)
inst2 = Label(
    root,
    text="1. Enter Data Points by completing X and Y fields and click Add",
    font=("Roboto", 11),
).place(x=540, y=40)
inst3 = Label(
    root, text="or simply import your data set via Import CSV", font=("Roboto", 11)
).place(x=540, y=65)
inst4 = Label(
    root,
    text="2. Indicate the x value of interest, Choose Degree to use.",
    font=("Tahoma", 11),
).place(x=540, y=90)
inst5 = Label(
    root,
    text="3. Select data points according to chosen degree by Ctrl (or Shift)",
    font=("Tahoma", 11),
).place(x=540, y=115)
inst6 = Label(root, text=" + Left Click.", font=("Tahoma", 11)).place(x=550, y=140)
inst7 = Label(
    root,
    text="For better accuracy, choose data points closest to X",
    font=("Tahoma", 11),
).place(x=580, y=165)

xLabel = Label(inputFrame, text="Enter x value:").grid(
    row=1, column=0, pady=10, sticky=E
)
xField = Entry(inputFrame, width=20)
xField.grid(row=1, column=1, columnspan=2)

yLabel = Label(inputFrame, text="Enter y value:").grid(
    row=2, column=0, pady=10, sticky=E
)
yField = Entry(inputFrame, width=20)
yField.grid(row=2, column=1, padx=10, columnspan=2)

findLabel = Label(inputFrame, text="Enter data point to find (x):").grid(
    row=3, column=0, pady=10
)
findField = Entry(inputFrame, width=20)
findField.grid(row=3, column=1, padx=10, columnspan=2)

orderLabel = Label(inputFrame, text="Choose Degree:").grid(
...
tableFrame.place(relx=0.5, rely=0.5, anchor=E)
global columns
global table
...
lagrangescroll = ttk.Scrollbar(
    tableFrame, orient="vertical", command=lagrangetable.yview
)
    ...
    if len(xField.get()) == 0 or len(yField.get()) == 0:
        ...
        try:
            ...
            table.insert(
                ...
                index="end",
                text="",
                values=(float(xField.get()), float(yField.get())),
            )
...
def reset():
    ...
    if len(table.get_children()) != 0:
        response = messagebox.askquestion(
            "Delete Data", "All entries and results will be deleted. Proceed?"
        )
        if response == "yes":
    ...
    if len(findField.get()) == 0:
        ...
        try:
            ...
            if datapoints != len(table.selection()):
                ...
                plt.plot(
                    all_x, all_y, color="magenta", marker="H", markerfacecolor="cyan"
                )
                # plot data points used in calculation
                plt.plot(
                    x_axis, y_axis, color="blue", marker="o", markerfacecolor="cyan"
                )
                ...
                plt.plot(
                    [int_point + 10, 0], [yp, yp], color="green", linestyle="dashed"
                )
...
def fileOpen():
    ...
    filename = filedialog.askopenfilename(
        title="Open a File", filetype=(("CSV files", "*.csv"), ("All files", "*.*"))
    )
    if filename:
        try:
            ...
            try:
                ...
                messagebox.showerror(
                    "Import Failed",
                    "Some values in the file are invalid. All data must be float data type",
                )
                return None
            else:
                for row in df_rows:
...
# Buttons
addButton = Button(
    inputFrame, text="Add Data", padx=50, pady=10, bg="light grey", command=add
)
addButton.grid(row=6, column=0, pady=5)

delButton = Button(
    inputFrame, text="Delete Data", padx=45, pady=10, bg="light grey", command=delete
)
delButton.grid(row=6, column=1, pady=5)

importButton = Button(
    inputFrame, text="Import CSV", padx=45, pady=10, bg="light grey", command=fileOpen
)
importButton.grid(row=7, column=0, pady=5)

resetButton = Button(
    inputFrame, text="Empty Table", padx=45, pady=10, bg="light grey", command=reset
)
resetButton.grid(row=7, column=1, pady=5)

calcuButton = Button(
    inputFrame, text="Calculate", padx=100, pady=20, bg="light grey", command=calculate
)
